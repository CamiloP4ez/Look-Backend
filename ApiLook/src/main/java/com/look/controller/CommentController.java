...
@RestController
@RequestMapping("/api/v1") // Base path común
@Tag(name = "Comments", description = "Operations related to comments")
@SecurityRequirement(name = "bearerAuth") // Mayoría requiere auth
public class CommentController {
    ...
    public ResponseEntity<ApiResponseDto <CommentResponseDto >> createComment(
        ...
        ApiResponseDto<CommentResponseDto> response = new ApiResponseDto<>("Comment created successfully", HttpStatus.CREATED.value(), createdComment);
    ...
    // No necesita @SecurityRequirement ni @PreAuthorize
    public ResponseEntity<ApiResponseDto<List<CommentResponseDto>>> getCommentsByPostId(
        ...
        ApiResponseDto<List<CommentResponseDto>> response = new ApiResponseDto<>("Comments fetched successfully", HttpStatus.OK.value(), comments);
        return ResponseEntity.ok(response);
    }

    @Operation(summary = "Delete a comment", description = "Requires authentication. Only author or ADMIN/SUPERADMIN.")
    @DeleteMapping("/comments/{commentId}")
    @PreAuthorize("isAuthenticated()") // La lógica fina está en el servicio
    public ResponseEntity<Void> deleteComment(
     ...
     public ResponseEntity<ApiResponseDto<CommentResponseDto>> updateComment(
         ...
         ApiResponseDto<CommentResponseDto> response = new ApiResponseDto<>("Comment updated successfully", HttpStatus.OK.value(), updatedComment);
         return ResponseEntity.ok(response);
     }
