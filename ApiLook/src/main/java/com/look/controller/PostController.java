...
import org.springframework.security.access.prepost.PreAuthorize; // Para autorización fina
...
@SecurityRequirement(name = "bearerAuth") // Indica que se requiere autenticación Bearer
public class PostController {
    ...
    public ResponseEntity<ApiResponseDto<PostResponseDto>> createPost(@Valid @RequestBody PostRequestDto postRequestDto) {
        PostResponseDto createdPost = postService.createPost(postRequestDto);
        ApiResponseDto<PostResponseDto> response = new ApiResponseDto<>("Post created successfully", HttpStatus.CREATED.value(), createdPost);
    ...
    public ResponseEntity<ApiResponseDto<List<PostResponseDto>>> getAllPosts() {
        ...
        ApiResponseDto<List<PostResponseDto>> response = new ApiResponseDto<>("Posts fetched successfully", HttpStatus.OK.value(), posts);
    ...
    public ResponseEntity<ApiResponseDto<PostResponseDto>> getPostById(
            @Parameter(description = "ID of the post to retrieve", required = true, in = ParameterIn.PATH)
            @PathVariable String postId) {
        PostResponseDto post = postService.getPostById(postId);
        ApiResponseDto<PostResponseDto> response = new ApiResponseDto<>("Post fetched successfully", HttpStatus.OK.value(), post);
        return ResponseEntity.ok(response);
    }
     ...
     // @PreAuthorize("hasPermission(#postId, 'Post', 'WRITE') or hasRole('ADMIN') or hasRole('SUPERADMIN')") // Ejemplo con permisos más finos (necesitaría implementar PermissionEvaluator)
     // Usaremos la lógica en el servicio por ahora
     public ResponseEntity<ApiResponseDto<PostResponseDto>> updatePost(
         ...
         ApiResponseDto<PostResponseDto> response = new ApiResponseDto<>("Post updated successfully", HttpStatus.OK.value(), updatedPost);
         return ResponseEntity.ok(response);
     }
     ...
     public ResponseEntity<Void> deletePost(
         ...
         // No devolver ApiResponseDto para NO_CONTENT
    ...
    // --- Endpoints para Likes ---
     ...
     // --- Endpoints para Comments (Añadir aquí) ---
     // GET /api/v1/posts/{postId}/comments
     // POST /api/v1/posts/{postId}/comments
     // DELETE /api/v1/posts/{postId}/comments/{commentId} (Con permisos adecuados)
